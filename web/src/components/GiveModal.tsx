...
const transformInput = (formValues: any, serverItems: any) => {
  ...
  return { ...formValues, type: 'send', items };
}
...
function ReceiveModal() {
  ...
  const headerText = "Bạn muốn cho gì?";
  ...
  useEffect(() => {
    ...
    if (isError) {
      ...
      toast({
        ...
        // description: mutateCreatePost?.error.toString() as string
      })
                      ...
                      <Field name="note">
                        {({ field, form }: { field: any, form: any }) => (
                          <FormControl isInvalid={form.errors.name && form.touched.name}>
                            ...
                            <InputGroup>
                              <Textarea {...field} resize="vertical" size="sm" placeholder="Ghi chú" />
                            </InputGroup>
                            <FormErrorMessage>{form.errors.name}</FormErrorMessage>
                          </FormControl>
                        )}
